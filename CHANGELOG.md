...
- **重构 AI 工具架构，解决控制权移交问题**
  - 原架构：AI工具返回结构化指令，主插件尝试用 `filter.on_llm_response()` 钩子拦截（失败）
  - 新架构：AI工具直接调用主插件方法，由主插件完成所有操作
  - 修改内容：
    - 删除 `_register_llm_response_hook()` 和 `_handle_tool_result()` 方法
    - 新增 `ai_start_game()`、`ai_join_game()`、`ai_check_status()` 供AI工具调用
    - 简化AI工具逻辑，移除游戏状态处理代码
  - 优势：控制流清晰，操作（禁言、消息）直接在主插件执行，可靠性高
  - 对用户无影响，原有自然语言交互功能正常工作
