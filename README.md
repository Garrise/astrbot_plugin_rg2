...
**注意**：`状态`和`帮助`指令放入了`左轮`指令组，避免与其他插件冲突。如果输入`/左轮`会显示指令组的树形结构。
...
### 管理员功能
- **装填子弹（指定数量）**：`/装填 [数量]` - 指定1-6发子弹，仅限管理员
- **走火控制**：`/走火开` 或 `/走火关` - 控制随机走火功能，仅限管理员

### 所有用户功能
- **装填子弹（随机）**：`/装填` - 随机装填1-6发子弹，所有人可用
- **射击**：`/开枪` - 扣动扳机参与游戏
- **状态查询**：`/左轮 状态` - 查看当前游戏状态（剩余子弹、当前弹膛）
- **帮助**：`/左轮 帮助` - 显示完整的使用说明和游戏规则

### 用户功能
- **射击**：`/开枪` - 扣动扳机参与游戏
- **状态查询**：`/状态` - 查看当前游戏状态（剩余子弹、当前弹膛）
- **帮助**：`/帮助` - 显示完整的使用说明和游戏规则
