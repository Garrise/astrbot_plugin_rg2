...
[![Version](https://img.shields.io/badge/version-1.1.1-green.svg)](https://github.com/piexian/astrbot_plugin_rg)
...
| `ai_trigger_delay` | AI工具触发延迟（秒） | 5 | 任意正整数 |
...
### AI触发器工具
1. **`start_revolver_game`** - 游戏启动触发器
   - **触发条件**："来玩左轮手枪"、"轮盘赌"、"再来一局"、"start game"
   - **权限说明**：仅管理员可指定子弹数量，普通用户随机装填
   - **处理方式**：触发主插件处理游戏初始化、状态设置和消息发送

2. **`join_revolver_game`** - 射击触发器
   - **触发条件**："我也要玩"、"我也要参加"、"开枪"、"shoot"、"我来试试"
   - **处理方式**：触发主插件处理射击逻辑、游戏结果和状态更新

3. **`check_revolver_status`** - 状态查询触发器
   - **触发条件**："游戏状态"、"状态"、"status"、"游戏情况"、"现在什么情况"
   - **处理方式**：触发主插件处理状态查询和信息返回

### 架构优势
- **职责明确**：AI工具仅负责触发，主插件负责所有业务逻辑
- **性能优化**：简化工具参数，减少不必要的数据传递
- **易于维护**：统一的错误处理和返回格式
